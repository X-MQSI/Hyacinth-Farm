...
```json
{
  ...
  "downsampleRate": 5      // 降采样率（每5个点取1个）
}
```

**降采样机制**

当查询时间段内的数据点超过1000个时，系统会自动进行降采样：
- 计算降采样率 = ⌈总数据点 / 1000⌉
- 均匀采样，确保覆盖完整时间段
- 例如：5000个数据点 → 降采样率为5 → 每5个点取1个 → 返回约1000个点
